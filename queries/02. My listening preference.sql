...
/*
Insight: Listening is fairly concentrated with;
Alternative Rock artists (Manchester Orchestra, Half Moon Run, Doves, Radiohead, The Boxer Rebellion, Other Lives),
Prog Rock/Metal artists (A Perfect Circle, Soen, TOOL, The Pineapple Thief, Porcupine Tree, Steven Wilson) all making the top 20 artists
...
/*
Insight: 
You return most often to Manchester Orchestra, A Perfect Circle, TOOL, Soen, and Half Moon Run, with 
Manchester Orchestra leading the pack at over 540 plays across three albums; A Black Mile to the Surface (255),
The Million Maks of God (222), The Valley of Vision (68).
*/
...
JOIN songs so ON so.song_id = st.song_id
JOIN artists ar ON ar.artist_id = so.artist_id
GROUP BY so.song_title, ar.artist_name
HAVING COUNT(st.stream_id) >= 10
ORDER BY skip_rate_percentage DESC;
/*
Insight:
Songs by Soen, Manchester Orchestra, and Ben Howard, are frequent plays but skipped before completion in 70–100% of plays — 
with Deceiver standing out as never finished once, 
and The Wolf showing both high play count (33 plays) and high skip rate (88% unfinished)
*/
